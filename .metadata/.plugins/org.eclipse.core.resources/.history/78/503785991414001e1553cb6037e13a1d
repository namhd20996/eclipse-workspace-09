...
<body>
	...
	<div class="container">
		<form class="row g-3" action="labs3-ctl" method="post"
		...
		<ul>
			<li>Họ và tên: <%=staff.getHoTen()%></li>
			<li>Hình ảnh: <%=staff.getHinhAnh()%></li>
			<li>Ngày sinh: <%=staff.getNgaySinh()%></li>
			<li>Giới tính: <%=staff.isGioiTinh() ? "Nam" : "Nữ"%></li>
			<li>Quốc tịch: <%=staff.getQuocTich()%></li>
			<li>TT Hôn nhân: <%=staff.isTtHonNhan() ? "Đã kết hôn" : "Chưa kết hôn"%></li>
			<li>Sở thích: <%=staff.getSoThich()%></li>
			<li>Ghi chú: <%=staff.getGhiChu()%></li>
			<li><img alt="" src="images/<%=staff.getHinhAnh() %>"> </li>
		</ul>
		<%
		}
		%>
